...
public class RemoteThread {
    ...
    private final DebuggeeAgent server;

    private static final Supplier<Boolean> TRUE_CONDITION = () -> true;

    private Supplier<Boolean> stopCondition;

    public RemoteThread(java.lang.Thread thread, DebuggeeAgent server) {
        this.lock = new Object();
        this.frames = new LinkedList<>();
        this.data = new Thread();
        data.setId((int) thread.getId());
        data.setName(thread.getName());
        this.server = server;
    ...
    public void onTemplateNode(ResolveEvent event) {
        ...
        var node = event.getTemplateNode();
        if (node != null) {
            if (node.isText()) {
