...
public class SourceTemplateRegistry {
    ...
    public SourceTemplateRegistry(BreakpointsRegistry breakpointsRegistry,
            SourceReferenceRegistry sourceReferenceRegistry, Engine engine) {
     ...
     */
    public SourceTemplateRegistry(BreakpointsRegistry breakpointsRegistry,
            SourceReferenceRegistry sourceReferenceRegistry, Engine engine, List<String> basePaths,
    ...
    public RemoteSource getSource(String templateId, Source previousSource) {
        ...
        /*
         * if (previousSource != null) { // Try to infer source path relative to the
         * previous source location String path =
         * previousSource.getPath().replace("\\", "/"); var existingBasePaths =
         * getExistingBasePaths(); if (!existingBasePaths.isEmpty()) { source =
         * findSource(templateId, path, existingBasePaths); if (source != null) { return
         * source; } } source = findSource(templateId, path, basePaths); if (source !=
         * null) { return source; } }
         *
         * var existingBasePaths = getExistingBasePaths(); if
         * (!existingBasePaths.isEmpty()) { source = findSource(templateId, "",
         * existingBasePaths); if (source != null) { return source; } }
         *
         * for (var entry : templateUriToSource.entrySet()) { var uri = entry.getKey();
         * if (uri.getSchemeSpecificPart().endsWith(templateId)) {
         * templateIdToSource.put(templateId, entry.getValue()); return
         * entry.getValue(); } }
         */
        return null;
    }
    ...
    private URI getGuesseddSourceUri(String templateId, Source previousSource) {
        ...
        existingUris.addAll(templateIdToSource.values().stream().map(RemoteSource::getUri).collect(Collectors.toSet()));
    ...
    /*
     * private List<String> getExistingBasePaths() { if
     * (templateUriToSource.isEmpty()) { return Collections.emptyList(); } return
     * templateUriToSource.keySet().stream() // transforme URI en Path .map(uri -> {
     * try { return Paths.get(uri).getParent().toString(); } catch
     * (IllegalArgumentException e) { // Cas des URI relatives comme file:src/...
     * String path = uri.getSchemeSpecificPart().replace("\\", "/"); int index =
     * path.lastIndexOf('/'); if (index != -1) { return path.substring(0, index); }
     * return path; }
     *
     * }) // .distinct() // supprime les doublons .sorted() // trie
     * .collect(Collectors.toList()); }
     *
     * private Source findSource(String templateId, String path, List<String>
     * basePaths) { for (var basePath : basePaths) { int index = path.isEmpty() ? 0
     * : path.indexOf(basePath); if (index != -1) { String sourcePath =
     * getValidSourcePath( (path.isEmpty() ? basePath + "/" : path.substring(0,
     * index + basePath.length())) + templateId); if (sourcePath != null) { Source
     * source = new Source(); source.setPath(sourcePath);
     * templateIdToSource.put(templateId, source); return source; } } } return null;
     * }
     */
