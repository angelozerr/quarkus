...
import io.quarkus.qute.parser.template.NodeKind;
import io.quarkus.qute.parser.template.Parameter;
import io.quarkus.qute.parser.template.ParameterInfo;
import io.quarkus.qute.parser.template.ParametersInfo;
import io.quarkus.qute.parser.template.Section;
import io.quarkus.qute.parser.template.SectionKind;
import io.quarkus.qute.parser.template.SectionMetadata;
...
public abstract class LoopSection extends Section {
    ...
    private static final ParametersInfo PARAMETER_INFOS = ParametersInfo.builder() //
            .addParameter(ALIAS, ParameterInfo.EMPTY) //
            .addParameter(IN, ParameterInfo.EMPTY) //
    ...
    private synchronized List<SectionMetadata> loadMetadata() {
        ...
        if (prefix == null) {
            ...
            metadata = METADATA.stream() //
                    .map(m -> new SectionMetadata(prefix + m.getName(), m.getType(), m.getDescription())) //
