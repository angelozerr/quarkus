...
import io.quarkus.qute.parser.NodeBase;

public abstract class Node extends NodeBase<Node> implements io.quarkus.qute.TemplateNode.Origin {
    ...
    public String getContent() {
        TemplateNode template = getOwnerTemplate();
        if (template != null) {
    ...
    // ========== Origin interface implementation ==========

    @Override
    public int getLine() {
        TemplateNode template = getOwnerTemplate();
        if (template != null) {
            return template.getLineNumber(getStart());
        }
        return 1;
    }
    ...
    public int getLineCharacterStart() {
        TemplateNode template = getOwnerTemplate();
        if (template != null) {
            int lineStartOffset = template.getLineStartOffset(getLine());
            return getStart() - lineStartOffset + 1;
        }
        return 1;
    }
    ...
    public int getLineCharacterEnd() {
        ...
        if (template != null) {
            int lineStartOffset = template.getLineStartOffset(getLine());
            return getEnd() - lineStartOffset + 1;
        }
        return 1;
    }
    ...
    public String getTemplateGeneratedId() {
        // For now, same as templateId
        return getTemplateId();
    }

    @Override
    public Optional<Variant> getVariant() {
        // TODO: Add variant support to TemplateNode
